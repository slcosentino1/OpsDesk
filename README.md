...
ReAct graph with three tools: `search_docs` (AskDoc), `lookup_ticket`, and `create_ticket` (in-memory ticket store). HITL on create and evaluation are not implemented yet.
...
python -m opsdesk "Open a ticket: I cannot connect to VPN after sleep. Requester ana@company.com"
```

Seed tickets in memory: `TCK-101` (open), `TCK-102` (closed), `TCK-103` (in progress). New tickets get the next id (`TCK-104`, …). They reset every process start. There is no confirmation step (HITL) yet.
